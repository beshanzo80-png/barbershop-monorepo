...
>(({ className, error, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'flex h-10 w-full items-center justify-between rounded-md bg-bg-tertiary border border-border-default',
      ...
      'aria-invalid:focus:ring-error aria-invalid:border-error',
  ...
  >
    {children}
    <SelectPrimitive.Icon className="ml-2 h-4 w-4 text-text-muted transition-transform duration-200">
      <ChevronDown />
...
>(({ className, children, position = 'popper', ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        'relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-md border border-border-default bg-bg-elevated text-text-primary shadow-lg',
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    className={cn(
      ...
      'data-[highlighted]:outline-none data-[highlighted]:bg-gold-subtle data-[highlighted]:text-gold-primary',
      className
    )}
    {...props}
  >
